...
+(NSInteger) getNextID : (NSString *) entityName
{
	...
	if(val)
	{
		...
		val = [NSNumber numberWithInt:(int)newID];
   ...
   NSExpression *attributeToFetch = [NSExpression expressionForKeyPath:DELEGATE.COREIDENTIFIER];
   ...
   NSArray *results = [[self defaultContext] executeFetchRequest:request error:nil];
    ...
    [dictID setObject:[NSNumber numberWithInt:(int)newID] forKey:entityName];
	return newID;
}
...
+(id) RelationgetObject: (NSString *) entityName withQueryString:(NSString *) queryString queryArguments:(NSArray *) arguments sortBy:(NSDictionary *)sortKeys
{
    NSArray *result	=	 [self getObjects:entityName inContext:[self defaultContext] withQueryString:queryString queryArguments:arguments sortBy:sortKeys];
    NSLog(@"%@",result);
...
//    [request            setFetchBatchSize:5];
	...
	if([sortDescs count] == 0)//Default is sort by ID ascending
	{
		...
		NSArray			*defSorts	=	[NSArray arrayWithObjects:@"serverID",DELEGATE.COREIDENTIFIER, nil];
     //   NSArray	*defSorts	=	[NSArray arrayWithObjects:@"serverID",@"id", nil];
...
+(NSArray *) getObjectspredict: (NSString *) entityName inContext:(NSManagedObjectContext *) ctx withQueryString:(NSString *) queryString queryArguments:(NSArray *) arguments sortBy:(NSDictionary *) sortKeys cacheName: (NSString *) cacheName
{
    NSFetchRequest		*request	=	[[NSFetchRequest alloc] init];
    NSEntityDescription *entity		=	[NSEntityDescription entityForName:entityName inManagedObjectContext:ctx];
